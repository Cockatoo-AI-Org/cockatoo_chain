...
class TestCockatooChain(unittest.TestCase):
  ...
  @patch('cockatoo_chain.utils.model_a.base.pyaudio')
  @patch('cockatoo_chain.utils.model_a.base.wave')
  def test_open_ai_wrapper_live_2_text(
      self, mock_wave, mock_pyaudio):
    """Test method `wrapper.live_2_text(...)`."""
    test_lang = LangEnum.cn
    test_record_time_sec = 1
    test_output_audio_file_path = 'test_output_audio_file_path'
    open_ai_wrapper = open_ai.OpenAIWrapper(test_lang)
    open_ai_wrapper.audio_2_text = mock.MagicMock()
    mock_p = mock_pyaudio.PyAudio.return_value
    print(f'mock_p={mock_p}')
    mock_stream = mock_p.open.return_value
    mock_stream.read.side_effect = [b'123']

    response = open_ai_wrapper.live_2_text(
    ...
    mock_wave.open.assert_called_with(
      test_output_audio_file_path, 'wb')
    open_ai_wrapper.audio_2_text.assert_called_with(
      test_output_audio_file_path)
    self.assertEqual(
      response, open_ai_wrapper.audio_2_text.return_value)
